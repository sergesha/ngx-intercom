...
describe('IntercomService', () => {
  ...
  it('should get content equal to afterward pushed value', inject([IntercomService], (service: IntercomService) => {
    service.read(['test']).subscribe(data => {
      expect(data.content).toEqual('The Test After');
    });
    service.push('test', 'The Test After');
  }));

  it('should read multiple values and get values pushed before', inject([IntercomService], (service: IntercomService) => {
  ...
  it('should remove content and get undefined value', inject([IntercomService], (service: IntercomService) => {
  ...
  it('should not send duplicate values if it\'s not intended', inject([IntercomService], (service: IntercomService) => {
    service.read(['test'])
      .pipe(
        take(5),
        skip(4)
    ...
    service.push('test', 'The Test Duplicate True', true);
